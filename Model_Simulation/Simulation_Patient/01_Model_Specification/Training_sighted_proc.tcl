...
set ofhead semobj_neurocomp_sighted_withEmpty_mVerbal
set trainfile semobj_t10_sighted_less_NEW1_moreVerbal
set testfile semobj_t10_sighted_less_NEW1_moreVerbal
set tmode ORDERED
set testcrit 0
set ZeroR 0
set ntstxp 258
...
for {set i 1} {$i<= $nsub} {incr i} {
		    ...
		    for {set j 0} {$j<=5} {incr j} {
			 ...
			 setObj weightDecay [expr (6-$j)*$lrate/10000]
			
			     for {set u 0} {$u<=19} {incr u} {
				 setObj "visualhid.unit(${u}).block(4).learningRate"  [expr [getObj "visualhid.unit(${u}).block(4).learningRate"]*(6-$j)/10]
