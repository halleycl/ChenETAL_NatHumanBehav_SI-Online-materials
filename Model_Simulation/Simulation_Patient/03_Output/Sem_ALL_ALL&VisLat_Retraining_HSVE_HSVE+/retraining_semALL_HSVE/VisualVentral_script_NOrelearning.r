...
nlevel = 10 #total levels of lesion
prop<-0.1  #lesion prop step
crit<-0.5
nrep<-10   #total number of retraining iteration
retstep<-500  #retraining stepwise
matA1<-read.table("Lexicon_target.txt",header=F)
stpA1<-2
stpA2<-2
stpB<-4
fnameh1<-"semobj_neurocomp_sighted_withEmpty_mVerbal_trained_lexOutp_pnam_delWtSem"
fnamee1<-".txt"
fnameOut2<-"_descrep"
fnameOut1<-"_acc"
initImp<-"0"   ##specify the initial level of impairment severity 0, 0.1,0.2, ..., 1.0, note to use the exact character
...
for (j in 1:nlevel) {
  ...
  for (i in 1:n) {
    ...
    matB<-read.table(paste(fnameh1,specify_decimal(prop*j,1),"_",initImp,fnameh3,i,fnamee1,sep=""), header = F)
	...
	stdDP<-sim_Stddp(matA1,matB,paste(fnameh1,specify_decimal(specify_decimal(prop*j,1)),"_",initImp,fnameOut2,"_StdDP",fnameh3,i,fnamee1,sep=""),stpA1,stpB,savetxt=F)
   ...
   write.table(accTotal,file=paste(n,"subjs_",fnameh1,specify_decimal(prop*j,1),"_",initImp,fnameOut1,fnamee1,sep=""),col.names=F, row.names=F)
   write.table(outTotal,file=paste(n,"subjs_",fnameh1,specify_decimal(prop*j,1),"_",initImp,fnameOut2,"_Response",fnamee1,sep=""),col.names=F, row.names=F)
   write.table(aBasTotal,file=paste(n,"subjs_",fnameh1,specify_decimal(prop*j,1),"_",initImp,fnameOut1,"_animalBasMean",fnamee1,sep=""),col.names=F, row.names=F)
   write.table(aSpecTotal,file=paste(n,"subjs_",fnameh1,specify_decimal(prop*j,1),"_",initImp,fnameOut1,"_animalSpecMean",fnamee1,sep=""),col.names=F, row.names=F)
   write.table(mBasTotal,file=paste(n,"subjs_",fnameh1,specify_decimal(prop*j,1),"_",initImp,fnameOut1,"_manmadeBasMean",fnamee1,sep=""),col.names=F, row.names=F)
   write.table(mSpecTotal,file=paste(n,"subjs_",fnameh1,specify_decimal(prop*j,1),"_",initImp,"_manmadeSpecMean",fnamee1,sep=""),col.names=F, row.names=F)
   
   write.table(rbind(colMeans(aBasTotal[1:24,]),colMeans(aSpecTotal[1:24,]),colMeans(mBasTotal[25:48,]),colMeans(mSpecTotal[25:48,])),file=paste(n,"subjs_",fnameh1,specify_decimal(prop*j,1),"_",initImp,"_BasSpecItemMean",fnamee1,sep=""),col.names=F, row.names=F)
